    ...
    <aside className="w-72 h-[calc(100vh-5rem)] fixed left-0 top-20 z-30 flex flex-col glass-panel border-r border-white/10 bg-[#020204]/80 backdrop-blur-2xl">
            ...
            <div key={idx}>
              {group.title !== '主菜单' && (
                <div className="flex items-center gap-3 px-4 mb-3">
                  ...
                  <div className="text-[12px] font-black text-white uppercase tracking-[0.25em] drop-shadow-[0_0_8px_rgba(255,255,255,0.4)] whitespace-nowrap">
                ...
                {group.items.map((item) => {
                  ...
                  return (
                    <button
                      ...
                      className={`
                        w-full flex items-center gap-4 px-5 py-3.5 rounded-xl transition-all duration-300 text-[13px] font-black uppercase tracking-[0.15em] group relative border
                        ${isActive 
      ...
      <div className="p-4 border-t border-white/10">
        <button 
           ...
           className="w-full flex items-center gap-4 px-5 py-4 rounded-xl text-white/30 hover:text-white hover:bg-white/5 transition-all text-[12px] font-black uppercase tracking-widest"
        >
       ...
       <div className="fixed inset-0 z-[99999] flex items-center justify-center bg-black/80 backdrop-blur-sm animate-in fade-in">
          ...
          <div className="relative w-[600px] h-[70vh] bg-[#12141a] border border-white/10 rounded-[2.5rem] shadow-2xl flex flex-col overflow-hidden">
             <div className="p-8 border-b border-white/10 flex justify-between items-center bg-[#161618]">
                <h3 className="text-xl font-black text-white uppercase tracking-widest">管理中心</h3>
             ...
             <div className="flex-1 overflow-y-auto p-8 bg-[#0f0f11] custom-scrollbar">
                <div className="space-y-6">
                   <div className="flex justify-between items-center mb-2 px-1">
                      <span className="text-[10px] font-black text-white/20 uppercase tracking-[0.3em]">声音库管理 ({voices.length})</span>
                      <button 
                        onClick={() => { setShowSettings(false); onNavigate(Page.VOICE_CLONING); }}
                        className="flex items-center gap-2 text-[10px] font-black text-spark-accent hover:text-blue-400 uppercase tracking-widest"
                      >
                        <Plus size={14} /> 新增声音
                      </button>
                   </div>
                      ...
                      {voices.map(voice => (
                         <div key={voice.id} className="flex items-center gap-4 p-4 rounded-2xl bg-white/[0.02] border border-white/5 group hover:bg-white/[0.05] hover:border-white/10 transition-all">
                            ...
                            <div className="flex-1 min-w-0">
                               <div className="text-base font-black text-white truncate tracking-tight">{voice.name}</div>
                               <div className="text-[10px] text-white/30 font-black uppercase tracking-widest mt-1">{voice.isCustom ? '自定义克隆' : '系统预设音色'}</div>
                            </div>
                            <button 
